...
will have dimensions
$m \times \lfloor n / 2 \rfloor$, 
$m \times \left(n - \lfloor n / 2 \rfloor - 1 \right)$,
$\lfloor m /2 \rfloor \times n$ or
$\left(m - \lfloor m /2 \rfloor -1 \right) \times n$
.

Therefore, the number of columns or rows - 
depending on if we are row splitting or column splitting -
in the problem strictly decreases with each recursive call 
as long as $n,m > 0$. So \texttt{algorithm4} either returns
a location at some point, or eventually examines
a subproblem with a non-positive number of columns or rows.
...
To do this we will focus only on the case of column splitting.
We can do this as \texttt{algorithm4} is invariant under a matrix transpose,
so long as we also change the way we split the matrix - from column
splitting to row splitting or vice versa;
therefore the proof we give for the columns generalises to the
case where we are row splitting by considering the same argument
but under a matrix transpose.
Just prior to this,
it must examine a subproblem of size
$m \times 1$ or $m \times 2$.
If the problem is of size $m \times 1$, and we are column splitting,
then calculating the maximum of the central column
is equivalent to calculating the maximum of the entire problem.
Hence, the maximum that the algorithm finds must be a peak,
and it will halt and return the location. 
If however we are row splitting - and m>1 - then can perform the row split 
and will have a new array of shape $m' \times 1$ so can apply the logic 
previously discussed. If instead we are row splitting
when $m = 1$ then we can consider the transpose of this problem and we have 
the same case as when we are column splitting with a matrix of size $m \times 1$.

If the problem has dimensions $m \times 2$,
whilst we are column splitting, then there are two possibilities:
either the maximum of the central column is a peak
(in which case the algorithm will halt and return the location),
or it has a strictly better neighbor in the other column
(in which case the algorithm will recurse
on the non-empty subproblem with dimensions $m \times 1$,
thus reducing to the previous case).
If instead we are row splitting then whilst m>2 we can just perform
row split and have a matrix of dimensions $m' \times 2$. If $m = 2$
Then we can apply the same logic as shown above but for rows instead of
columns (this can be seen by agai considering the transpose of the problem). 
...
then that location must have the best value in column $c_1$ or row $r_1$ of the sub problem,
and must have been a peak within some recursive subproblem.
For this proof we shall consider the case that a column split occurs when the location
returned stops being a peak. However, this argument would also apply to a row split
due to the algorithm acting the same as a transposed row split.
Assume, for the sake of contradiction,
that $(r_1, c_1)$ is not also a peak within the original problem.
Then as the location $(r_1, c_1)$ is passed up the chain of recursive calls,
it must eventually reach a level where it stops being a peak.
...
$$val(r_1, c_1) < val(r_1, c_2) \le val(r_2, c_2) < val(r_2, c_1)$$
But in order for \texttt{algorithm1} to return $(r_1, c_1)$ as a peak,
the value at $(r_1, c_1)$ must have been the greatest value that we have seen,
making $val(r_1, c_1) \ge val(r_2, c_1)$.
Hence, we have a contradiction.
\end{quote}
